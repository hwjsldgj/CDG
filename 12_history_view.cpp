...
static void ComputeStats() {
    ...
    LOG_DEBUG(std::string("统计完成：总数=") + std::to_string(g_stats.totalCount) +
              "，最高分=" + std::to_string(g_stats.maxScore) +
              "，平均分=" + std::to_string(g_stats.averageScore));
    LOG_EXIT();
}
...
static void LoadHistoryList() {
    ...
    do {
        ...
        if (pos != std::wstring::npos) {
            try {
                ...
                LOG_DEBUG(std::string("加载文件：") + filename + "，得分：" + std::to_string(score));
                fileCount++;
            } catch (...) {
                LOG_WARN(std::string("解析文件失败：") + filename + "（得分行格式错误）");
                g_historyList.push_back({filename, timeStr, 0, false});
            }
        } else {
            LOG_WARN(std::string("解析文件失败：") + filename + "（未找到“得分：”标识）");
            g_historyList.push_back({filename, timeStr, 0, false});
        }
        file.close();
    ...
    LOG_INFO(std::string("历史记录加载完成，共 ") + std::to_string(fileCount) + " 项");
    ComputeStats();
    LOG_EXIT();
}
...
void History_HandleInput() {
    ...
    if (GetAsyncKeyState(g_config.KEY_CONFIRM) & 0x8000) {
        if (g_selectedIndex < (int)g_historyList.size()) {
            ...
            if (item.valid) {
                LOG_INFO(std::string("历史记录按键：回车，选择文件 ") + item.filename + "，进入详情");
                FileDetail_Init(item.filename);
                g_state.gameMode = GameState::FILE_DETAIL;
            } else {
                LOG_WARN("历史记录：选中无效文件，无法查看详情");
            }
        }
        Sleep(150);
        LOG_EXIT();
